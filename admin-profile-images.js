...
async function load(){
  ...
  const rows=usersResult.data||[],ids=rows.map(r=>r.profile_id).filter(Boolean);let avatarRows=[];
...
if(typeof document!=='undefined'){document.addEventListener('click',event=>{if(event.target&&event.target.closest&&event.target.closest('#openAdminPage'))setTimeout(load,50);});document.addEventListener('kronang:auth-signed-in',()=>setTimeout(load,150));setTimeout(load,1200);}
