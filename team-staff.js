...
function renderStaffRows(rows){
  ...
  if(!section){
    ...
    section.id='teamStaffSection';
    section.className='team-staff-section';
  ...
  (rows||[]).map(buildTeamStaffMember).filter(function(member){return member.name;}).forEach(function(member){
    ...
    const role=document.createElement('span');role.textContent=member.role||'Ledare';
...
async function loadTeamStaff(){
  ...
  const {data,error}=await window.kronangSupabase.from('team_staff').select('display_name, staff_role, phone, email, avatar_url, sort_order').eq('team',profile.team).eq('is_active',true).order('sort_order',{ascending:true}).order('display_name',{ascending:true});
