...
function buildStaffSaveRequest(member){
  ...
  return {
    ...
    p_avatar_url:(item.avatarUrl||'').trim(),
...
function createStaffEditor(member,onSaved,onCancel){
  const item=member||{id:null,name:'',role:'',description:'',phone:'',email:'',avatarUrl:'',sortOrder:100};
  ...
  form.addEventListener('submit',async ev=>{
    ...
    const request=buildStaffSaveRequest({id:item.id,name:form.elements.name.value,role:form.elements.role.value,description:form.elements.description.value,phone:form.elements.phone.value,email:form.elements.email.value,avatarUrl:item.avatarUrl,sortOrder:item.sortOrder});
...
function renderStaffRows(rows,isAdmin){
  ...
  members.forEach((member,index)=>{
    ...
    if(isAdmin){const admin=document.createElement('div');admin.className='team-staff-admin-actions';const edit=document.createElement('button');edit.type='button';edit.textContent='Redigera';const up=document.createElement('button');up.type='button';up.textContent='↑';up.disabled=index===0;const down=document.createElement('button');down.type='button';down.textContent='↓';down.disabled=index===members.length-1;const remove=document.createElement('button');remove.type='button';remove.textContent='Ta bort';admin.append(edit,up,down,remove);card.appendChild(admin);
...
async function loadTeamStaff(){
  ...
  renderStaffRows(data||[],canAdminManageStaff(profile.role));
}
