(function(root){
  function smsUri(phone,body){const number=String(phone||'').replace(/[^+0-9]/g,'');const encoded=encodeURIComponent(String(body||''));const isiOS=/iPad|iPhone|iPod/.test((root.navigator&&root.navigator.userAgent)||'');return 'sms:'+number+(isiOS?'&body=':'?body=')+encoded;}
  function messageText(name,inviteUrl){return 'Kronängs IF Juniorlag\n\nHej '+String(name||'')+'!\nDu är inbjuden till lagets spelarapp.\n\nTryck på länken och välj e-postadress och lösenord för att aktivera ditt konto:\n'+inviteUrl+'\n\n/Kronängs IF';}
  function addStyles(){if(!root.document||document.getElementById('playerOnboardingInviteStyles'))return;const style=document.createElement('style');style.id='playerOnboardingInviteStyles';style.textContent='.player-onboarding-action{margin:24px auto 0;max-width:420px;padding-top:20px;border-top:1px solid rgba(214,182,95,.35)}.player-onboarding-action button{width:100%;min-height:52px;border:1px solid #d6b65f;border-radius:14px;background:#d6b65f;color:#080808;font-weight:900;letter-spacing:.03em}.player-onboarding-action button:disabled{opacity:.6}.player-onboarding-action-status{margin:10px 0 0;color:#f0b8b8;font-size:14px}.player-onboarding-action-status.connected{display:inline-block;padding:9px 14px;border:1px solid rgba(214,182,95,.6);border-radius:999px;color:#f4d97e;font-weight:850}';document.head.appendChild(style);}
  let role='',lastPlayerId='',ready=false;
  ...
  const api={smsUri:smsUri,messageText:messageText};if(typeof module!=='undefined'&&module.exports)module.exports=api;if(root.document){wait();document.addEventListener('kronang:auth-signed-in',function(){ready=false;lastPlayerId='';wait();});}root.KronangPlayerOnboardingInvite=api;
