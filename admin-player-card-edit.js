...
(function(root){
  ...
  async function openInlineEditor(profile,playerId,triggerButton){
    ...
    cancel.onclick=function(){form.remove();triggerButton.hidden=false;};
    form.onsubmit=async function(event){
      ...
      const cardName=selectedRosterCard&&selectedRosterCard.querySelector('.player-roster-card-title strong');
      if(cardName)cardName.textContent=displayName(payload.full_name,payload.nickname);
      form.remove();triggerButton.hidden=false;triggerButton.textContent='Sparat ✓';
  ...
  function injectAdminPlayerEditAction(){
    ...
    button.onclick=async function(){
      ...
      if(!playerId)return;
      const opened=await openInlineEditor(profile,playerId,button);
