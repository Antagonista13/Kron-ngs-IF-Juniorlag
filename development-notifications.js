...
async function loadUnread(){if(!root||!root.kronangSupabase)return[];const{data,error}=await root.kronangSupabase.from('development_notifications').select('id,event_type,entity_type,entity_id,created_at,read_at').is('read_at',null).order('created_at',{ascending:false});if(error){console.error('Kunde inte hämta utvecklingsnotiser:',error);return[];}return data||[];}
...
async function refresh(){const rows=await loadUnread();renderDot(rows);return rows;}
