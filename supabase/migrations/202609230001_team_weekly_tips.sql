...
create policy "team weekly tips readable by team"
on public.team_weekly_tips for select
to authenticated
using (
  exists (
    ...
    where p.id = (select auth.uid())
      ...
      and p.team = team_weekly_tips.team
      and p.role in ('admin','coach','player')
  )
);

drop policy if exists "admins insert team weekly tips" on public.team_weekly_tips;
create policy "admins insert team weekly tips"
on public.team_weekly_tips for insert
to authenticated
with check (
  exists (
    select 1 from public.profiles p
...
create policy "admins update team weekly tips"
on public.team_weekly_tips for update
to authenticated
using (
  exists (
    select 1 from public.profiles p
...
create policy "admins delete team weekly tips"
on public.team_weekly_tips for delete
to authenticated
using (
  exists (
    select 1 from public.profiles p
...
values ('weekly-tips', 'weekly-tips', false)
